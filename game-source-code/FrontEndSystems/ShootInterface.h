...
public:
	 ...
	 * @param repository The repository used to construct a the projectile object, used for testing
	 */
	virtual void Shoot(	Vector2D startPosition, Vector2D target, std::shared_ptr<Scene> scene,  const std::shared_ptr<RepositioryInterface> repository = Application::getGameRepository()) = 0;
...
protected:
	 ...
	 */
	virtual std::shared_ptr<Projectile> getProjectile(const std::shared_ptr<RepositioryInterface> repository) = 0;
